...
contract GuessGame {
    ...
    function pureFunc() internal pure {
        assembly {
            mstore(0x80, 1)
            mstore(0xa0, 2)
            mstore(0xc0, 32)
    ...
    function guess(
        ...
        uint256 y = (uint160(address(msg.sender)) +
            random01 +
            random02 +
            random03 +
            _random02) & 0xff;
        ...
        require(
            uint160(_random03) <
                uint160(0x0000000000fFff8545DcFcb03fCB875F56bedDc4)
        );
        (, bytes memory data) = address(uint160(_random03)).staticcall(
            "Fallbacker()"
        );
